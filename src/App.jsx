...
const translations = {
  RU: {
    ...
    beta: { 
      ...
      brand: 'Шатерфил', site: 'shutterfeel.com', tg: '@tpa3007', category: 'Streetwear', volume: '100к – 500к' }
  },
  EN: {
...
function PaymentWidget({ t }) {
  ...
  if (status === 'idle') {
    return (
      <button onClick={handleStartPayment} className="w-full max-w-xs h-16 bg-white border-2 border-black flex items-center justify-between px-6 group hover:bg-black transition-all shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] text-black cursor-pointer active:translate-x-1 active:translate-y-1 active:shadow-none overflow-hidden relative z-40">
  ...
  if (status === 'success') {
    return (
      <div className="w-full max-w-sm border-2 border-black bg-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] overflow-hidden text-black animate-in zoom-in-95 duration-300 relative z-40">
         ...
         <div className="p-12 text-center space-y-6 text-black">
  ...
  return (
    <div className="w-full max-w-sm border-2 border-black bg-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] overflow-hidden text-black animate-in slide-in-from-top-2 fade-in duration-300 relative z-40">
      <div className="bg-black text-white p-4 border-b-2 border-black flex justify-between items-center text-white">
      ...
      <div className="p-4 sm:p-6 bg-zinc-100">
        {status === 'processing' ? (
          ...
          <div className="space-y-4 text-black text-left">
            <div className="bg-white border-2 border-black p-4 space-y-4 shadow-sm text-black text-left">
              <div className="flex justify-between items-start text-black"><div className="space-y-1 text-left"><div className="text-[10px] font-black uppercase">GENESIS_NFT_#01</div><div className="text-[7px] font-bold uppercase opacity-40">Asset Status: Ready_to_Mint</div></div><Plus size={16} className="text-green-500" /></div>
...
function FittingTerminal({ t }) {
  ...
  if (!isExpanded) {
    return (
      <button onClick={() => setIsExpanded(true)} className="w-full max-w-sm h-16 bg-white border-2 border-black flex items-center justify-between px-6 group hover:bg-black transition-all shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-black cursor-pointer active:translate-x-1 active:translate-y-1 active:shadow-none overflow-hidden relative z-40 text-black">
        <div className="flex items-center gap-4 text-left pointer-events-none text-black">
  ...
  return (
    <div className="w-full max-w-sm border-2 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden relative z-40 text-black">
      <div className="bg-black text-white p-3 border-b-2 border-black flex justify-between items-center text-white">
        <div className="flex items-center gap-2">
  ...
  return (
    <footer className="bg-black text-white py-16 text-left relative z-10 border-t-2 border-black text-white">
      <div className={`${standardPadding} grid grid-cols-1 md:grid-cols-12 gap-12 mb-16 text-white`}>
        <div className="md:col-span-6 text-white"><span className="text-4xl md:text-5xl font-black tracking-tighter uppercase block mb-6 text-white">THYSS</span><p className="max-w-xs text-[12px] font-medium tracking-tight opacity-40 leading-relaxed text-white">{t.footer.desc} <br /> {t.footer.sub}</p></div>
  ...
  return (
    <div className="min-h-screen bg-white text-black flex flex-col items-center justify-center p-6 text-center animate-in zoom-in-95 duration-500">
      <div className="w-full max-w-lg space-y-12 text-black">
        <div className="relative text-black text-center"><h1 className="text-[15vw] font-black uppercase tracking-tighter leading-none opacity-10">404</h1><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full space-y-4 text-black text-center"><div className="inline-block p-4 bg-black text-[#CCFF00] border-4 border-black font-black uppercase text-xl tracking-widest">{t.notFound.denied}</div><p className="text-sm font-black uppercase tracking-[0.2em]">{t.notFound.error}</p></div></div>
        <div className="bg-zinc-50 border-2 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col sm:flex-row items-center gap-8 text-left text-black"><div className="shrink-0 relative group"><RealQRCode data="https://t.me/thysssystem" size={120} /></div><div className="space-y-4 text-black text-left"><div className="space-y-1"><span className="text-[10px] font-black uppercase tracking-widest opacity-40">Support Group</span><h3 className="text-xl font-black uppercase tracking-tighter">Emergency Link</h3></div><p className="text-[11px] font-bold opacity-60 uppercase leading-relaxed">{t.notFound.sub}</p><a href="https://t.me/thysssystem" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest underline decoration-[#CCFF00] hover:bg-[#CCFF00] px-1 transition-colors">@thysssystem <ArrowRight size={12} /></a></div></div>
  ...
  const handleSubmit = async (e) => { 
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Ошибка при отправке. Пожалуйста, попробуйте еще раз.');
        setStatus('idle');
      }
    } catch (error) {
      ...
      <main className={`pt-[72px] pb-12 md:pb-24 ${standardPadding} animate-in fade-in duration-700`}>
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 items-start mt-12">
          <div className="lg:col-span-5 space-y-12">
            <div className="space-y-6">
              <div className="w-fit px-2 py-1 bg-black text-white text-[9px] font-black uppercase tracking-widest text-white">PHASE_01_BETA</div>
              <h1 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-[0.82] text-black">JOIN THE <br /> <span className="bg-[#CCFF00] px-2 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] border-2 border-black text-black">CORE.</span></h1>
            ...
            <div className="pt-8 border-t border-black/10 text-black">
               <div className="flex items-center gap-4 text-black text-left">
                  ...
                  <div className="text-left"><div className="text-[10px] font-black uppercase">Telegram Channel</div><a href="https://t.me/thysssystem" target="_blank" rel="noopener noreferrer" className="text-[10px] font-bold opacity-40 uppercase hover:opacity-100 hover:text-blue-600 transition-all">@thysssystem</a></div>
               </div>
            </div>
          </div>
          <div className="lg:col-span-7">
            {status === 'idle' ? (
              <div className="bg-white border-4 border-black p-8 md:p-12 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] text-black text-left">
              ...
              <div className="bg-white border-4 border-black p-8 md:p-12 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] h-full flex flex-col items-center justify-center text-center space-y-8 py-24 min-h-[500px] text-black"><Loader2 size={48} className="animate-spin text-black opacity-20" /><div className="space-y-2"><h3 className="text-2xl font-black uppercase tracking-tighter">Syncing...</h3><p className="text-[10px] font-black opacity-40 uppercase tracking-widest">Phase 01 Security Handshake</p></div></div>
            ) : (
              <div className="bg-[#CCFF00] border-4 border-black p-12 md:p-20 text-center shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] animate-in zoom-in-95 duration-500 min-h-[500px] flex flex-col justify-center text-black text-center"><div className="w-24 h-24 bg-black text-[#CCFF00] rounded-full flex items-center justify-center mx-auto mb-10 shadow-[8px_8px_0px_0px_rgba(0,0,0,0.1)]"><Check size={48} strokeWidth={4} /></div><h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter mb-4 text-black">Access Granted</h2><p className="text-xs font-black uppercase tracking-widest text-black/60 mb-12">THYSS-PH1-GRANTED</p><button onClick={onBack} className="px-10 py-5 bg-black text-white font-black uppercase text-[10px] tracking-[0.3em] hover:bg-white hover:text-black border-2 border-black transition-all max-w-xs mx-auto w-full">{t.notFound.back}</button></div>
            )}
          </div>
        </div>
      </main>
  ...
  return (
    <div className="text-black font-sans bg-white overflow-x-hidden">
      ...
      <div className="pt-[72px]">
        <Marquee lang={lang} />
        <header className="border-b border-black"><div className={`w-full py-24 ${standardPadding} text-left`}><h1 className={`${headingBold} text-[18vw] md:text-[12vw] text-black`}>{t.hero.title1} <br /> {t.hero.title2} <br /> {t.hero.title3}</h1><div className="pt-12"><button onClick={onConnect} className="px-8 py-4 bg-[#CCFF00] text-black font-black uppercase text-xs sm:text-sm tracking-[0.2em] flex items-center gap-3 border-2 border-black hover:bg-black hover:text-white transition-all group shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] cursor-pointer text-black">{t.hero.cta} <ArrowRight size={18} /></button></div></div></header>
        
        {/* Добавлен scroll-mt-[72px] ко всем секциям с ID для правильной работы якорей */}
        <section className="bg-white border-b border-black py-16 text-left scroll-mt-[72px]"><div className={`w-full ${standardPadding}`}><div className="w-fit inline-block px-2 py-1 bg-black text-white text-[9px] font-black uppercase mb-6 text-white">{t.gateway.tag}</div><h2 className="text-6xl sm:text-9xl font-black uppercase tracking-tighter mb-12 text-black">{t.gateway.title}</h2><p className="text-2xl sm:text-5xl font-bold tracking-tight text-black">{t.gateway.desc}<span className="bg-[#CCFF00] px-2 text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] font-black uppercase">{t.gateway.highlight}</span> {t.gateway.sub}</p></div></section>
        
        <section id={t.nav.widget} className="border-b border-black bg-white overflow-hidden text-left scroll-mt-[72px]"><div className="flex flex-col lg:grid lg:grid-cols-12 w-full"><div className={`lg:col-span-6 py-16 ${standardPadding} flex flex-col justify-center`}><div className="w-fit inline-block px-2 py-1 bg-black text-white text-[9px] font-black uppercase mb-6 text-white">{t.widget.tag}</div><h2 className="text-6xl sm:text-8xl font-black uppercase tracking-tighter mb-10 text-black">{t.widget.title}</h2><p className="text-lg font-medium text-black/70 mb-10 text-black">{t.widget.desc}</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><CompactFeature icon={Layers} title={t.widget.f1} desc={t.widget.f1_desc} /><CompactFeature icon={Cpu} title={t.widget.f2} desc={t.widget.f2_desc} /></div></div><div className="lg:col-span-6 py-16 px-6 flex items-center justify-center border-t lg:border-t-0 lg:border-l border-black bg-zinc-50 relative lg:min-h-[450px]"><div className="absolute inset-0 opacity-10 bg-[radial-gradient(#000_1px,transparent_1px)] [background-size:30px_30px] pointer-events-none"></div><PaymentWidget t={t} /></div></div></section>
        
        <section id={t.nav.crm} className="border-b border-black bg-[#CCFF00] overflow-hidden text-left scroll-mt-[72px]"><div className="flex flex-col lg:grid lg:grid-cols-12 w-full"><div className={`lg:col-span-6 py-16 ${standardPadding} flex flex-col justify-center`}><div className="w-fit inline-block px-2 py-1 bg-black text-white text-[9px] font-black uppercase mb-6 text-white">{t.crm.tag}</div><h2 className="text-6xl sm:text-8xl font-black uppercase tracking-tighter mb-10 text-black">{t.crm.title}</h2><p className="text-lg font-medium text-black/70 mb-10 text-black">{t.crm.desc}</p><div className="grid grid-cols-2 gap-4"><CompactFeature icon={Target} title={t.crm.f1} desc={t.crm.f1_desc} /><CompactFeature icon={TrendingUp} title={t.crm.f2} desc={t.crm.f2_desc} /></div></div><div className="lg:col-span-6 py-16 px-6 flex items-center justify-center border-t lg:border-b-0 lg:border-l border-black bg-[#CCFF00]"><div className="w-full max-w-md border-2 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden text-black"><div className="bg-black text-white p-3 border-b-2 border-black flex justify-between items-center text-white"><span className="text-[9px] font-black uppercase tracking-widest text-white">{t.crm.feed}</span><Database size={14} className="text-[#CCFF00]" /></div><div className="p-6 space-y-4 text-black text-left"><div className="p-4 border-2 border-black bg-zinc-50 flex gap-4 items-center text-black"><div className="w-16 h-16 bg-black flex items-center justify-center border border-black shadow-[4px_4px_0px_0px_rgba(204,255,0,1)] text-[#CCFF00]"><Search size={24} className="animate-pulse text-[#CCFF00]" /></div><div className="text-left text-black"><div className="text-[10px] font-black uppercase">{t.crm.scanned}</div><div className="text-[8px] font-mono opacity-50">UQAs...3e9j_wallet</div></div></div>{[ { addr: '0x71...a3b', amount: '12,420 USDT', tags: ['WHALE', 'OG'] }, { addr: '0x32...f9e', amount: '840 USDT', tags: ['ACTIVE'] }, { addr: '0x9a...2c1', amount: '2,100 USDT', tags: ['OG'] } ].map(user => (<div key={user.addr} className="p-3 border border-black/10 flex justify-between items-center bg-[#FDFDFD] text-[10px] font-black uppercase leading-none text-black"><div className="flex flex-col gap-1 text-black text-left"><div className="flex items-center gap-2"><Users size={12} /> <span>{user.addr}</span></div><div className="flex gap-1 text-black text-left">{user.tags.map(tag => (<span key={tag} className={`text-[6px] font-black border px-1 ${tag === 'WHALE' ? 'bg-orange-50 border-orange-200 text-orange-600' : tag === 'OG' ? 'bg-purple-50 border-purple-200 text-purple-600' : 'bg-green-50 border-green-200 text-green-600'}`}>{tag}</span>))}</div></div><span className="text-black">{user.amount}</span></div>))}</div></div></div></div></section>
        
        <section id={t.nav.loyalty} className="border-b border-black bg-white overflow-hidden text-left scroll-mt-[72px]"><div className="flex flex-col lg:grid lg:grid-cols-12 w-full text-black"><div className={`lg:col-span-6 py-16 ${standardPadding} flex flex-col justify-center`}><div className="w-fit inline-block px-2 py-1 bg-black text-white text-[9px] font-black uppercase mb-6 text-white">{t.loyalty.tag}</div><h2 className="text-6xl sm:text-8xl font-black uppercase tracking-tighter mb-10 text-black">{t.loyalty.title}</h2><p className="text-lg font-medium text-black/70 mb-10 text-black">{t.loyalty.desc}</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><CompactFeature icon={Heart} title={t.loyalty.f1} desc={t.loyalty.f1_desc} /><CompactFeature icon={Share2} title={t.loyalty.f2} desc={t.loyalty.f2_desc} /></div></div><div className="lg:col-span-6 py-16 px-6 flex items-center justify-center border-t lg:border-b-0 lg:border-l border-black bg-zinc-50 relative min-h-[450px]"><NFTTerminal t={t} /></div></div></section>
        
        <section id={t.nav.fitting} className="border-b border-black bg-zinc-50 overflow-hidden text-left scroll-mt-[72px]"><div className="flex flex-col lg:grid lg:grid-cols-12 w-full text-black"><div className={`lg:col-span-6 py-16 ${standardPadding} flex flex-col justify-center`}><div className="w-fit inline-block px-2 py-1 bg-black text-white text-[9px] font-black uppercase mb-6 text-white">{t.fitting.tag}</div><h2 className="text-6xl sm:text-8xl font-black uppercase tracking-tighter mb-10 text-black">{t.fitting.title}</h2><p className="text-lg font-medium text-black/70 mb-10 text-black">{t.fitting.desc}</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-black"><CompactFeature icon={Maximize} title={t.fitting.f1} desc={t.fitting.f1_desc} /><CompactFeature icon={Heart} title={t.fitting.f2} desc={t.fitting.f2_desc} /></div></div><div className="lg:col-span-6 py-16 px-6 flex items-center justify-center border-t lg:border-b-0 lg:border-l border-black bg-white relative"><div className="absolute inset-0 opacity-5 bg-[linear-gradient(#000_1px,transparent_1px),linear-gradient(90deg,#000_1px,transparent_1px)] [background-size:40px_40px] pointer-events-none"></div><FittingTerminal t={t} /></div></div></section>
        
        <section id={t.nav.rates} className="border-b border-black text-left scroll-mt-[72px]"><div className="grid grid-cols-1 md:grid-cols-3 w-full border-t border-black bg-white">{plansData.map((p) => { const displayPrice = p.price; const isDark = p.dark; return (<div key={p.id} className={`relative p-12 border-b md:border-b-0 md:border-r border-black flex flex-col min-h-[700px] ${isDark ? 'bg-black text-white' : 'bg-white text-black'} last:border-b-0`}>{p.popular && (<div className="absolute top-0 right-12 -translate-y-1/2 bg-[#CCFF00] border-2 border-black px-4 py-1.5 text-[10px] font-black uppercase text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] whitespace-nowrap z-20">{t.pricing.popular}</div>)}<span className={`text-[11px] font-black uppercase tracking-[0.3em] mb-6 ${isDark ? 'text-white/40' : 'text-black/30'}`}>{p.name}</span><div className="mb-4">{displayPrice === 0 ? (<div className={`text-6xl sm:text-7xl font-black ${isDark ? 'text-white' : 'text-black'}`}>FREE</div>) : displayPrice === 'CUSTOM' ? (<div className={`text-6xl sm:text-7xl font-black ${isDark ? 'text-white' : 'text-black'}`}>{displayPrice}</div>) : (<div className="flex items-baseline gap-1"><span className={`text-6xl sm:text-7xl font-black ${isDark ? 'text-white' : 'text-black'}`}>{displayPrice}</span><span className={`text-xl font-black ${isDark ? 'text-white' : 'text-black'}`}>{t.pricing.currency}</span><span className={`text-[10px] font-bold opacity-50 ml-2 ${isDark ? 'text-white' : 'text-black'}`}>{t.pricing.perMonth}</span></div>)}</div>{p.promo && (<div className="mb-4 py-1.5 px-3 bg-white/10 border border-white/20 text-[9px] font-black uppercase tracking-wider text-[#CCFF00] w-fit">{p.promo}</div>)}<p className={`text-[12px] font-semibold mb-10 text-left opacity-70 ${isDark ? 'text-white/80' : 'text-black/60'}`}>{p.sub}</p><ul className={`space-y-4 mb-12 flex-grow uppercase font-bold text-[9px] ${isDark ? 'text-white' : 'text-black'}`}>{p.features.map(f => (<li key={f} className="flex items-start gap-3"><div className={`w-1.5 h-1.5 mt-1 flex-shrink-0 ${isDark ? 'bg-[#CCFF00]' : 'bg-black'}`}></div><span>{f}</span></li>))}</ul><button onClick={() => onConnect()} className={`w-full py-5 text-[10px] font-black uppercase border-2 transition-all cursor-pointer shadow-[6px_6px_0px_0px_rgba(0,0,0,0.1)] active:shadow-none active:translate-x-1 active:translate-y-1 ${isDark ? 'bg-white text-black border-white hover:bg-[#CCFF00]' : 'bg-black text-white border-black hover:bg-[#CCFF00] hover:text-black'}`}>{p.price === 'CUSTOM' ? t.pricing.contact : t.pricing.choose}</button></div>); })}</div></section>
