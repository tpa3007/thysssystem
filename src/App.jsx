...
// Здесь хранятся все надписи сайта. Если нужно поменять текст, искать его нужно именно здесь.
const translations = {
  RU: {
    // Навигационное меню
    ...
    mint: { 
      ...
      steps: [
        ...
        'Запись в блокчейн TON...',
        'Подтверждение владения...'
      ]
    },
    // Секция виртуальной примерки
    fitting: { 
  ...
  EN: {
    // Английские переводы (структура полностью дублирует русскую)
    ...
    beta: { 
      ...
      brand: 'Brand Name', site: 'Website / Social', tg: 'Telegram ID', 
      category: 'Niche / Category', volume: 'Monthly Sales',
      perks: [
        'USDT Acquiring with 0% Commission', 
        'On-chain Customer Analytics', 
        '30 Days of Full Access',
        'Personal support & priority updates'
      ],
      processing: 'Syncing...',
      helpTitle: 'Telegram Channel', helpSub: '@thysssystem',
      placeholders: { brand: 'Шатерфил', site: 'shutterfeel.com', tg: '@tpa3007', category: 'Streetwear', volume: '10K – 50K' }
    }
  }
};

// --- ВСПОМОГАТЕЛЬНЫЕ КОМПОНЕНТЫ (Маленькие блоки) ---

// Функция для отрисовки QR-кода (берет данные и создает картинку через сторонний сервис)
...
// Плитка с описанием какой-то функции (иконка + заголовок + текст)
...
// Компонент бегущей строки вверху сайта
    ...
    <div className="w-full bg-black overflow-hidden border-b border-black">
      <div className="flex items-center pt-1 pb-1.5">
        <div className="flex animate-marqueeInfinite whitespace-nowrap">
          {/* Дублируем контент 4 раза, чтобы строка была бесконечной */}
      ...
      {/* CSS-анимация для движения строки */}
      <style>{`
...
// --- КОМПОНЕНТ: NavigationBlock (Меню вверху страницы) ---
function NavigationBlock({ onNavigateHome, onConnect, t, lang, setLang, standardPadding, isSimplified = false }) {
  return (
    <nav className="sticky top-0 w-full z-50 bg-white border-b border-black py-4">
      <div className={`w-full ${standardPadding} flex justify-between items-center`}>
        ...
        {/* Если мы не на упрощенной версии (например, не на странице регистрации) */}
        {!isSimplified ? (
          <>
            {/* Навигационные ссылки (скрываются на мобилках) */}
            ...
            {/* Переключатель языка и кнопка Бета-тест */}
          ...
          /* Кнопка "Назад" для страницы беты */
...
// --- ВИДЖЕТЫ (Интерактивные элементы в середине лендинга) ---

// Виджет оплаты: демонстрирует Whitelabel-процесс, таймер и результат
function PaymentWidget({ t }) {
  const [status, setStatus] = useState('idle'); // Состояние: idle (кнопка), payment (ожидание), success (готово), error (ошибка)
  const [walletAddress, setWalletAddress] = useState('UQD-STABLE-CHECKPOINT-TON-ADDR-777');
  const [timeLeft, setTimeLeft] = useState(30000); // 30 секунд в миллисекундах
  const [isCopied, setIsCopied] = useState(false);
  
  const amount = 142.50;
  const currency = "USDT";

  // Сброс и запуск процесса
  const handleStartPayment = () => {
    setTimeLeft(30000);
    setStatus('initializing');
  };

  // Эффект имитации загрузки
  useEffect(() => {
  ...
  // Таймер обратного отсчета (миллисекунды)
  useEffect(() => {
    let timer;
  ...
  // Красивое форматирование времени: 00:00
  const formatTime = (ms) => {
  ...
  // Режим ожидания: большая черная кнопка
  if (status === 'idle') {
    return (
  ...
  // Режим успеха: зеленая иконка "Галочка"
  if (status === 'success') {
    return (
      <div className="w-full max-w-sm border-2 border-black bg-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] overflow-hidden text-black animate-in zoom-in-95 duration-300 relative z-50">
         <div className="bg-black text-white p-4 border-b-2 border-black flex justify-between items-center">
            <div className="flex items-center gap-3"><div className="w-2.5 h-2.5 rounded-full bg-[#CCFF00]"></div><span className="text-[10px] font-black uppercase tracking-[0.1em] text-white">{t.payment.success}</span></div>
            <button onClick={() => setStatus('idle')} className="hover:text-[#CCFF00] transition-colors p-1"><X size={16}/></button>
         </div>
         <div className="p-12 text-center space-y-6">
            <div className="w-20 h-20 bg-[#CCFF00] border-2 border-black rounded-full flex items-center justify-center mx-auto shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] text-black">
  ...
  // Режим ошибки (таймаут): красное окно
  if (status === 'error') {
    return (
      <div className="w-full max-w-sm border-2 border-black bg-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] overflow-hidden text-black animate-in zoom-in-95 duration-300 relative z-50">
         <div className="bg-red-600 text-white p-4 border-b-2 border-black flex justify-between items-center">
            <div className="flex items-center gap-3"><div className="w-2.5 h-2.5 rounded-full bg-white animate-pulse"></div><span className="text-[10px] font-black uppercase tracking-[0.1em] text-white">{t.payment.error}</span></div>
         ...
         <div className="p-12 text-center space-y-6">
            ...
            <div className="space-y-2">
               <h4 className="text-2xl font-black uppercase tracking-tighter leading-none text-black">{t.payment.error}</h4>
               <p className="text-[10px] font-bold opacity-40 uppercase text-black">{t.payment.timeout}</p>
            </div>
  ...
  // Основной виджет оплаты (QR-код и адрес)
  return (
      ...
      <div className="p-6">
        {status === 'initializing' ? (
          ...
          <div className="space-y-6">
            {/* Блок с QR-кодом */}
            <div className="flex gap-5 items-center bg-zinc-50 border-2 border-black p-5 shadow-sm">
               ...
               <div className="flex flex-col text-left space-y-1 text-black">
            ...
            {/* Имитация открытия кошелька: сразу переводит в статус Успех */}
            <button 
...
// Виджет NFT: имитация выпуска цифровой награды с этапами протокола
function NFTTerminal({ t }) {
  ...
  // Эффект переключения текстовых шагов загрузки
  useEffect(() => {
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-grow grid grid-cols-1 gap-1">
                  {[
                    ...
                    <div key={i} className="p-1 border-b border-black/5 flex justify-between text-[7px] font-black uppercase text-black">
...
// Виджет примерки: маскот, ползунки параметров и ИИ-подбор размера
function FittingTerminal({ t }) {
  ...
  // Имитация ИИ-анализа параметров
  const handleScan = () => { 
    setIsScanning(true); 
    setTimeout(() => { 
      setIsScanning(false); 
  ...
  // Развернутый вид виджета (с ползунками)
  return (
    <div className="w-full max-w-sm border-2 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden relative z-50 text-black">
      ...
      <div className="p-4 bg-zinc-100 space-y-3">
        {/* Карточка товара */}
        <div onClick={() => window.open('https://shutterfeel.com', '_blank')} className="bg-white border-2 border-black p-2 flex items-center justify-between group hover:bg-[#CCFF00] transition-all cursor-pointer">
          <div className="flex items-center gap-2">
            ...
            <div className="text-left text-black">
              ...
              <div className="text-[9px] font-black uppercase leading-none text-black">shutterfeel.com Bauhaus T-Shirt</div>
            </div>
          </div>
          <ExternalLink size={10} className="opacity-20 group-hover:opacity-100 text-black" />
        </div>

        {/* Зона визуализации (место для будущего маскота) */}
        {!isCalculated && (
          <div className="bg-white border-2 border-black relative h-32 overflow-hidden flex items-center justify-center">
             ...
             <div className="relative flex flex-col items-center">
               <div className="w-16 h-16 rounded-full border-2 border-dashed border-black/20 flex items-center justify-center relative">
                 ...
                 <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-black/5 to-[#CCFF00]/40 border-2 border-black animate-float-slow flex items-center justify-center">
        ...
        {/* Экран результата */}
        {isCalculated ? (
          <div className="bg-white border-2 border-black p-6 text-center space-y-4 animate-in zoom-in-95 duration-300">
             <div className="text-[10px] font-black uppercase opacity-40 text-black">Your Recommended Size:</div>
             <div className="text-7xl font-black text-black tracking-tighter">{result}</div>
             ...
             <div className="space-y-3 pt-2">
                <button 
                  ...
                  disabled={isAdded}
                  className={`w-full py-4 font-black uppercase text-[10px] tracking-widest border-2 border-black transition-all flex items-center justify-center gap-2 transform active:scale-95 ${
                    isAdded ? 'bg-[#CCFF00] text-black shadow-none' : 'bg-black text-white hover:bg-[#CCFF00] hover:text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)]'
                  }`}
                >
                ...
                <button 
                  ...
                  className="flex items-center justify-center gap-2 mx-auto text-[8px] font-black uppercase opacity-30 hover:opacity-100 hover:text-black transition-all group text-black"
                >
          ...
          /* Ввод параметров через ползунки */
          <div className="grid grid-cols-2 gap-2 text-black">
             {[{ id: 'height', label: 'Height (cm)', val: measurements.height, min: 150, max: 200 }, { id: 'weight', label: 'Weight (kg)', val: measurements.weight, min: 40, max: 130 }, { id: 'chest', label: 'Chest (cm)', val: measurements.chest, min: 80, max: 130 }, { id: 'waist', label: 'Waist (cm)', val: measurements.waist, min: 60, max: 120 }].map((field) => (
               <div key={field.id} className="bg-white border border-black p-2 space-y-1">
                 <div className="flex justify-between items-center text-[7px] font-black uppercase opacity-50 text-black"><span>{field.label}</span><span className="text-black opacity-100">{field.val}</span></div>
...
// --- CORE BLOCKS (Основные блоки страницы) ---
...
function FooterBlock({ onNavigateError, standardPadding, t }) {
  return (
    <footer className="bg-black text-white py-16 text-left relative z-10 border-t-2 border-black">
      <div className={`${standardPadding} grid grid-cols-1 md:grid-cols-12 gap-12 mb-16`}>
        ...
        <div className="md:col-span-6 grid grid-cols-2 gap-8 md:gap-10">
          ...
          <div className="flex flex-col gap-3 md:gap-4 uppercase text-[8px] md:text-[9px] font-black tracking-widest text-white/60">
            <span className="border-b border-white/10 pb-2 text-white/70 font-black">{t.footer.support}</span>
          ...
          {/* Ссылки Соцсетей */}
          <div className="flex flex-col gap-3 md:gap-4 uppercase text-[8px] md:text-[9px] font-black tracking-widest text-white/60">
            <span className="border-b border-white/10 pb-2 text-white/70 font-black">{t.footer.social}</span>
      ...
      {/* Копирайт и партнерская ссылка Shutterfeel */}
...
// Страница 404 (заглушка для неразработанных разделов)
  ...
  return (
    <div className="min-h-screen bg-white text-black flex flex-col items-center justify-center p-6 text-center animate-in zoom-in-95 duration-500">
      <div className="w-full max-w-lg space-y-12 text-black">
        ...
        <button onClick={onBack} className="px-10 py-5 bg-black text-white font-black uppercase text-[11px] tracking-[0.3em] border-2 border-black hover:bg-[#CCFF00] hover:text-black transition-all shadow-[6px_6px_0px_0px_rgba(0,0,0,0.2)] cursor-pointer">
           {t.notFound.back}
        </button>
      </div>
    </div>
...
// Страница Бета-доступа (форма сбора заявок)
function BetaAccessPage({ onBack, t, lang, setLang, standardPadding }) {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-black selection:bg-[#CCFF00] selection:text-black">
      ...
      <main className={`pt-6 md:pt-8 pb-12 md:pb-24 ${standardPadding} animate-in fade-in duration-700`}>
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          {/* Левая часть: заголовки и преимущества */}
          <div className="lg:col-span-5 space-y-12">
            <div className="space-y-6">
              <div className="w-fit px-2 py-1 bg-black text-white text-[9px] font-black uppercase tracking-widest">PHASE_01_BETA</div>
              <h1 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-[0.82]">JOIN THE <br /> <span className="bg-[#CCFF00] px-2 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] border-2 border-black">CORE.</span></h1>
              <div className="space-y-4 pt-4">
                 {t.beta.perks.map((perk, i) => (
                   <div key={i} className="flex items-center gap-4 group text-black">
                 ...
                 <form onSubmit={handleSubmit} className="space-y-10">
                    ...
                    <div className="space-y-4 text-black">
                      <div className="flex items-center gap-2 opacity-80">
                        <BarChart3 size={14} />
                        <label className="text-[10px] font-black uppercase tracking-widest">{t.beta.volume}</label>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-black">
                        {volumeOptions.map(val => (
                          <button key={val} type="button" onClick={() => setFormData({...formData, volume: val})} className={`py-3 text-[10px] font-black border-2 border-black transition-all ${formData.volume === val ? 'bg-black text-white' : 'bg-white text-black hover:bg-[#CCFF00]'}`}>{val}</button>
                        ))}
                      </div>
                    </div>
...
// --- LANDING CONTENT (Сборка главной страницы) ---
...
function LandingContent({ onNavigateError, onConnect, t, lang, setLang, standardPadding }) {
  ...
  const starterFeatures = t.pricing.features.starter;
  const businessFeatures = t.pricing.features.business;
  const enterpriseFeatures = t.pricing.features.enterprise;

  const plansData = [ 
    { id: 'starter', name: t.pricing.starter, price: 0, features: starterFeatures, sub: t.pricing.sub1 }, 
    { id: 'business', name: t.pricing.business, price: t.pricing.businessPrice, features: businessFeatures, dark: true, beta: true, sub: t.pricing.sub2, popular: true, promo: t.pricing.betaFree }, 
    { id: 'enterprise', name: t.pricing.enterprise, price: "CUSTOM", features: enterpriseFeatures, beta: true, sub: t.pricing.sub3 } 
  ];

  return (
    <div className="text-black font-sans bg-white overflow-x-hidden">
      {/* Шапка и Бегущая строка */}
...
export default function App() {
  const [page, setPage] = useState('landing'); // Управление текущей страницей (landing, beta, 404)
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-black overflow-x-hidden selection:bg-[#CCFF00] selection:text-black">
      {/* Рендерим страницу в зависимости от состояния */}
      {page === 'landing' ? (
      ...
      {/* Глобальные стили анимаций и скроллбара */}
      <style>{`
