...
function FittingTerminal({ t }) {
  ...
  if (!isExpanded) {
    return (
      <button onClick={() => setIsExpanded(true)} className="w-full max-w-sm h-16 bg-white border-2 border-black flex items-center justify-between px-6 group hover:bg-black transition-all shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-black cursor-pointer active:translate-x-1 active:translate-y-1 active:shadow-none overflow-hidden relative z-40">
  ...
  const handleSubmit = async (e) => { 
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Мы не используем alert() в соответствии с правилами среды разработки (CSP/iframe)
                 ...
                 <form onSubmit={handleSubmit} className="space-y-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      {[
                        ...
                        <div key={field.id} className="space-y-3 group text-black">
                    ...
                    <div className="space-y-4 text-black">
                      <div className="flex items-center gap-2 opacity-80"><BarChart3 size={14} /><label className="text-[10px] font-black uppercase tracking-widest">{t.beta.volume}</label></div>
...
function LandingContent({ onNavigateError, onConnect, t, lang, setLang, standardPadding }) {
  ...
  return (
    <div className="text-black font-sans bg-white overflow-x-hidden">
      ...
      {/* Стыковка пиксель в пиксель: высота шапки h-[72px] = pt-[72px] */}
